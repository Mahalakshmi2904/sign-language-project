...
# 🔥 Load Numbers Model (Correct File Name)
with open("isl_numbers_model.pkl", "rb") as f:
    model = pickle.load(f)
...
while True:
    ...
    if result.multi_hand_landmarks:
        ...
        # 🔥 Wrist Normalization (Same as Alphabets)
        wrist = lm[0]
        data = []
        ...
        predicted_char = str(model.predict(data)[0])
        ...
        if predicted_char != current_char:
            ...
            if time.time() - char_start_time >= STABLE_TIME:
                ...
                cursor += 1
                current_char = ""  # allow repeated numbers

    else:
        current_char = ""

    # 🔥 Multi-line Sentence Display
    ...
    cv2.imshow("ISL Numbers Detection", frame)
